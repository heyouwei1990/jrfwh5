<template>
  <div class="th_my_sort">
    <div class="mySortDataBox" >
      <mt-radio
        v-if="selectList && selectList.list"
        align="right"
        v-model="orderBy"
        :options="selectList.list"
        @click.native="click">
      </mt-radio>
    </div>
    </div>
</template>

<script>
export default {
  props: {
    select: {
      type: Object
    },
    selectList: {
      type: Object
    }
  },
  watch: {
    selectList: {
      handler (val) {
        this.setSelectList(val)
      },
      deep: true
    },
    orderBy (val) {
      this.$set(this.select, 'orderBy', val)
      this.$set(this.select, 'orderText', this.orderText)
      this.$emit('close')
    }
  },
  data () {
    return {
      orderBy: '',
      orderText: ''
    }
  },
  components: {
  },
  computed: {

  },
  mounted () {
    this.setSelectList()
  },
  created () {
  },
  methods: {
    setSelectList () {
      this.selectList && this.selectList.list && this.selectList.list.forEach(element => {
        element.value = element.id + ''
        element.label = element.name
      })
    },
    click (e) {
      e.target.className === 'mint-radiolist-label' && (this.orderText = e.target.innerText)
    }
  }
}
</script>

<style lang="less" scoped>
.th_my_sort{
   height: 100%;
    background-color: @cf;
    position: relative;
    .mySortDataBox{
      height: 100%;
      overflow: auto;
      & /deep/ .mint-radiolist{
        .mint-cell{
          height: 2.5rem;
          .mint-cell-title{
            height: 2.5rem;
            line-height: 2.5rem;
            .mint-radio-input:checked + .mint-radio-core {
              display: inline-block;
              background-color: @cD14E33;
              border-color: @cD14E33;
              &:after{
                content: "";
                border-radius: 0;
                border: 2px solid @cf;
                border-left: 0;
                border-top: 0;
                background-color: @cD14E33;
                width: 4px;
                height: 8px;
                top: 3px;
                left: 6px;
                transform: rotate(45deg) scale(1);
              }
            }
            .mint-radio-core{
              display: none;
              &:after{
                transform: rotate(45deg) scale(0);
              }
            }
          }
        }
      }
    }
  }

</style>
